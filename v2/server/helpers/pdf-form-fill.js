/**
 * This a ready code copied form the samples code for hummusjs library
...
function startModifiedDictionary(handles, originalDict, excludedKeys) {
  ...
  Object.getOwnPropertyNames(originalDictJs).forEach(
    (element, index, array) => {
...
function updateOptionButtonValue(handles, fieldDictionary, value) {
  ...
  if (isWidget || !fieldDictionary.exists('Kids')) {
    ...
    const modifiedDict = startModifiedDictionary(handles, fieldDictionary, {
      V: -1,
      AS: -1,
    });
    let appearanceName;
    if (value === null) {
      ...
      const apDictionary = handles.reader
    ...
    const kidsArray = handles.reader
    ...
    if (value === null) {
      ...
      // grab the non off value. that should be the yes one
      const widgetDictionary = handles.reader
    ...
    const fieldsReferences = writeKidsAndEndObject(
      handles,
      modifiedDict,
      kidsArray
    );
    ...
    for (let i = 0; i < fieldsReferences.length; ++i) {
      ...
      const modifiedFieldDict = startModifiedDictionary(handles, sourceField, {
        AS: -1,
      });
      if (value === i) {
...
function writeAppearanceXObjectForText(
  ...
  const originalAppearanceContent = getOriginalTextFieldAppearanceStreamCode(
    handles,
    fieldsDictionary
  );
  let before = '';
  ...
  if (textOptions) {
    ...
    const textDimensions = textOptions.font.calculateTextDimensions(
      text,
      textOptions.size
    );
...
function writeFieldWithAppearanceForText(
  ...
  if (appearanceInField) {
    ...
    const modifiedDict = startModifiedDictionary(handles, sourceField, {
      AP: -1,
    });
...
function updateTextValue(
  ...
  const modifiedDict = startModifiedDictionary(
    handles,
    fieldDictionary,
    fieldsToRemove
  );
...
function updateFieldWithValue(
  ...
  const localFlags = fieldDictionary.exists('Ff')
...
function writeFieldAndKids(
  ...
  const modifiedFieldDict = startModifiedDictionary(handles, fieldDictionary, {
    Kids: -1,
  });
...
function writeFilledField(
  ...
  const localFieldNameT = fieldDictionary.exists('T')
...
function writeFilledForm(handles, acroformDict) {
  const modifiedAcroFormDict = startModifiedDictionary(handles, acroformDict, {
    Fields: -1,
  });
...
function fillForm(writer, data, options) {
  ...
  const acroformDict = catalogDict.exists('AcroForm')
